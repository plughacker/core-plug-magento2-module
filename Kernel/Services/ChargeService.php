...
use PlugHacker\PlugAPILib\Models\GetChargeResponse;
...
use PlugHacker\PlugCore\Webhook\Services\ChargeOrderService;
...
class ChargeService
{
    ...
    public function captureById($chargeId, $amount = 0)
    {
        try {
            ...
            $charge = $chargeRepository->findByPlugId(
    ...
    {
        $order = (new OrderRepository())->findByPlugId(
        ...
        $this->logService->info(
            "Capturing charge on Plug - {$charge->getPlugId()->getValue()}"
        );
        ...
        if ($resultApi instanceof GetChargeResponse) {
            if (!$charge->getStatus()->equals(ChargeStatus::paid())) {
                $this->logService->info(
                    "Pay charge - {$charge->getPlugId()->getValue()}"
                );
                $charge->pay($amount);
            }
     ...
     * @return ServiceResponse
     */
    public function cancelJustAtPlug(Charge $charge)
    {
        $this->logService->info("Call just Charge cancel");

        $this->logService->info(
            "Cancel charge on Plug - " . $charge->getPlugId()->getValue()
        );
