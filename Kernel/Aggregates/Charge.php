...
use PlugHacker\PlugCore\Kernel\Exceptions\InvalidOperationException;
...
final class Charge extends AbstractEntity implements ChargeInterface
{
    ...
    public function pay($amount)
    {
        ...
        if ($this->getTransactionRequests()->getPaidAmount() > $this->getAmount()) {
            $this->status = ChargeStatus::overpaid();
            $this->setPaidAmount($this->getTransactionRequests()->getPaidAmount());
        }
    }
    ...
    public function addTransaction(Transaction $newTransaction)
    {
        ...
        foreach ($transactions as $transaction) {
            if ($transaction->getPlugId()->equals(
                $newTransaction->getPlugId()
            )
            ) {
                return $this;
            }
    ...
    public function updateTransaction(Transaction $updatedTransaction, $overwriteId = false)
    {
        $transactions = $this->getTransactions();
        foreach ($transactions as &$transaction) {
            if ($transaction->getPlugId()->equals($updatedTransaction->getPlugId())) {
    ...
    public function getCustomerId()
    {
        ...
        return $this->getCustomer()->getPlugId();
    }
