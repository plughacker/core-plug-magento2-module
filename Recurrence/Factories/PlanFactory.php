...
use PlugHacker\PlugCore\Recurrence\ValueObjects\DueValueObject;
...
class PlanFactory implements FactoryInterface
{
    ...
    private function setPlugId($postData)
    {
        if (!empty($postData['plan_id'])) {
            $this->plan->setPlugId(new PlanId($postData['plan_id']));
            return;
        }
    }
